...
The [prospective local-selector diagnostic](../studies/prospective-proxy-1/README.md) generated six answers to two new synthetic technical questions. Greyscope's lowest score matched the independent quality reviewer's preferred answer on each question. Both preferred answers scored 100% AI / 0% mixed / 0% human in fresh Basic 4.9b scans. Each best quality group contained just one answer, so the frozen selector had no eligible alternative. Agreement with a quality preference did not establish a detector reduction; the four unscanned alternatives remain unmeasured.

Its separately frozen adaptive continuation organized each explanation around the object or cached response being reused. A fresh reviewer preferred the DNS revision, which still scored 100% AI; it preferred the Python original, leaving that revision unscanned. Both cases remain reported, and the experimental instruction was not promoted. Keeping a fix beside the condition that explains it remains useful for clarity, but this trial supplies no detector benefit.
