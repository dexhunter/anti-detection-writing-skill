...
The detector objective concerns prose. Preserve independently supplied code and standalone commands while polishing the message; code changes need source-backed technical reasons. Return the complete answer for review. The coordinator freezes the declared code ranges and derived prose before review or scanning, keeping inline identifiers and all explanatory text in the prose.
...
For `prose_only`, also inspect both code-range manifests and derived prose files. Approve exclusions only when they contain independent code and retain every explanatory sentence and inline identifier. Review code correctness and its agreement with the prose separately from detector performance. Bind the review to both full-message hashes and both prose hashes; the controller additionally requires `code_ranges_approved: true` for eligible prose scans.

## Detector worker

Receive the exact frozen detector input, its declared scope and full-message association, reserved attempt ID, detector mode, permitted entry transform and available allowance. Use the user's signed-in Chrome and requested Basic Scan for GPTZero. Read the current UI and quota before submitting. Submit that entire measured input once; wait for completion, read back its effective text and save the actual native results, warnings and private screenshots. For `prose_only`, code stays in the separately reviewed message and is not appended to the scan. See [receipt contract](receipt-contract.md). No wording changes, credit purchases, limit bypasses, posting or result invention. Do not navigate away while processing. A missing, failed or uncertain result is a recorded outcome, never zero or 100 by assumption. A worker for another detector follows that detector's native units; it cannot satisfy a GPTZero-specific posting condition.
