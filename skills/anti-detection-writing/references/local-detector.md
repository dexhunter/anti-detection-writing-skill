...
## Additional open detectors

The [September 10 proxy comparison](https://github.com/dexhunter/anti-detection-writing-skill/tree/main/studies/open-detector-proxy-1) exercised pinned Oculus v2 and Greyscope v2 MLX 4-bit models on 16 archived prose inputs forming 10 dependent historical comparisons, plus two historical human controls. At a predeclared 0.01 native-score decrease, Greyscope recognized all three historical GPTZero reductions and suggested decreases on two of seven ties. Oculus recognized two reductions and suggested decreases on four ties. Two successful comparisons reused the same business topic; this was selected development evidence, not an unseen accuracy test.

Greyscope is the more promising exploratory diagnostic here, but its large decrease on a technical repair/editing example accompanied an unchanged 100% GPTZero result. Establish prospective agreement before using it to select rewrites automatically. Its calibrated ordinal AI involvement is not GPTZero confidence. Preserve its author preprocessing and full prompt, count tokens before any truncation, and retain original and normalized text separately. The study contains the tested wrappers and native outputs; this installed CLI remains the pinned Desklib implementation.

The subsequent [prospective diagnostic](https://github.com/dexhunter/anti-detection-writing-skill/tree/main/studies/prospective-proxy-1) generated three answers for each of two new synthetic technical questions. Greyscope's minimum matched the independent reviewer's preferred answer on each question; both preferred answers then scored 100% AI in fresh GPTZero Basic scans. The predeclared quality groups were singletons, so the selector had no eligible alternative and added no observed benefit. These two baselines do not measure the unscanned alternatives' GPTZero scores or establish a ranking correlation.
