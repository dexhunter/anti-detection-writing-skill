...
REVISION = "e6b41a2159c78d480738317c3ea6a680073a7b32"
MAX_TOKENS = 512
MODEL_MANIFEST = Path(__file__).resolve().parent / "model-manifest.json"
...
def verify_model(model_dir, manifest_path=MODEL_MANIFEST):
    ...
    require(isinstance(files, dict) and {"config.json", "model.safetensors",
            "tokenizer_config.json", "tokenizer.json"} <= set(files), "Model manifest lacks required files")
    ...
    def __init__(self, model_dir, device):
        ...
        class Detector(torch.nn.Module):
            ...
            def forward(self, input_ids, attention_mask):
                ...
                return self.classifier(pooled.to(self.classifier.weight.dtype))
...
def run(model_dir, manifest, output, device="cpu", backend_factory=OculusBackend,
