...
- Compare how the original and reference organize explanations, connect sentences, introduce components and place qualifications. Record one or two distinct, concrete differences that could improve this reader's understanding. Each must include an exact original span, an exact reference span, a proposed operation and its reader benefit. Use structural operations rather than personality adjectives. Do not copy reference wording or introduce its examples, facts, identity, opinions or experiences. Do not change actors, certainty, stance, evidence, prerequisites or technical specificity.
- Freeze the one or two justified deltas as the sole editorial treatment. Do not draft sentences for the target, prescribe novelty for its own sake or add more interventions. If no suitable difference exists, return inapplicable with reasons and stop; do not force a second difference when only one is justified.

### Stage B: one fresh complete realization

- Dispatch one different fresh writer with this policy, the original, reader request, complete claim contract, protected items, source evidence, exact reference and frozen one-or-two-delta record. Do not pass the preparation conversation, other answers, scores or prior attempts. Actual isolated agent invocations are required; role headings in one response do not implement separate contexts. Keep inherited backend and reasoning settings unchanged.
- Write the full answer once, realizing the original meaning through the one or two frozen operations. Ordinary new syntax and vocabulary are allowed. Keep useful wording where it fits; do not merely append style instructions, mechanically shuffle sentences or require all sentences to change. Preserve every required claim, useful qualification and protected item. Respect the requested format and word ceiling, including title and caption when requested. This complete case contract overrides any ordinary permission for scope cuts.
