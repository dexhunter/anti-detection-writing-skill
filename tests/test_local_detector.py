"""Synthetic local-detector contract checks; these fixtures are not model results."""
...
SCRIPT = Path(__file__).resolve().parents[1] / "skills/anti-detection-writing/scripts/local_detector.py"
...
class LocalDetectorTests(unittest.TestCase):
    def setUp(self):
        ...
        files = {}
        for name in ("config.json", "model.safetensors", "tokenizer_config.json", "spm.model"):
    ...
    def test_capacity_includes_special_tokens_and_never_truncates(self):
        for words, expected in ((766, "complete"), (767, "failed")):
    ...
    def test_model_file_drift_and_unverified_extra_are_rejected(self):
        ...
        (self.model_dir / "tokenizer.json").write_text("Unverified preferred tokenizer")
